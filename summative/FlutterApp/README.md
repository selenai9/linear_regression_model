...
> **Note:** hosted on Render's free tier, which spins down after inactivity — the first request after idle time may take 30–60 seconds to respond while it wakes up.
...
2. Install dependencies:
   ...
   flutter pub get
   ```
3. `lib/api_service.dart` is already pointed at the live Render API (`ApiConfig.baseUrl`) — no changes needed unless you want to test locally instead.
4. Run:
   ```bash
   flutter run
   ```
